...
from ipywidgets import HTML, HBox, Output,VBox
...
class NGLTrajectory2D(NGLWidgets):
    def __init__(self, trajectory):
        ...
        self.slider_amplitude_description=widgets.Label("Oscillation amplitude",layout=self.layout_description)
        ...
        self.button_optic_description=widgets.HTMLMath(r"Wave type",layout=self.layout_description)
        self.button_optic.observe(self.on_band_change_honey,"value")
        # kx_array=ky_array = np.linspace(-1.2*np.pi+eps, 1.2*np.pi-2*eps, 61) # Avoid division b2 zero
        ...
        self.ky_array_honey=np.linspace(-1.5*4*np.pi/(3*np.sqrt(3)),1.5*4*np.pi/(3*np.sqrt(3)),61)

        self.KX_honey, self.KY_honey= np.meshgrid(self.kx_array_honey,self.ky_array_honey)
        self.idx_x_honey = 15
        self.idx_y_honey = 15
        ...
        self.button_lattice=widgets.RadioButtons(options=['square','honeycomb'],value='square')
        self.button_lattice_description=widgets.HTMLMath(r"Lattice",layout=self.layout_description)
        ...
        self.slider_C1_description=widgets.HTMLMath(r"$C_1$",layout=widgets.Layout(width="50px"))
        ...
        self.slider_C2_description=widgets.HTMLMath(r"$C_2$",layout=widgets.Layout(width="50px"))
        ...
        self.slider_C1_description_honey=widgets.HTMLMath(r"$C_1$",layout=widgets.Layout(width="50px"))
        ...
        self.slider_C2_description_honey=widgets.HTMLMath(r"$C_2$",layout=widgets.Layout(width="50px"))
        ...
        self.slider_C3_description_honey=widgets.HTMLMath(r"$C_3$",layout=widgets.Layout(width="50px"))

        self.button_longitudinal=widgets.RadioButtons(options=['longitudinal','transverse'],value='longitudinal')
        self.button_longitudinal_description=widgets.HTMLMath(r"Wave direction",layout=self.layout_description)
        # self.button_longitudinal_honey=widgets.RadioButtons(options=['longitudinal','transverse'],value='longitudinal')
    ...
    def on_force_change(self,*args):
        if self.button_lattice.value=='square':
            ...
            self.line_GM_trans.set_data(np.linspace(0,20,21,dtype='int32'),  self.w_GM_trans)
            self.line_MX_trans.set_data(np.linspace(20,40,21,dtype='int32'), self.w_MX_trans)
            self.line_XG_trans.set_data(np.linspace(40,60,21,dtype='int32'), self.w_XG_trans)

            self.line_GM_long.set_data(np.linspace(0,20,21,dtype='int32'),  self.w_GM_long)
            self.line_MX_long.set_data(np.linspace(20,40,21,dtype='int32'), self.w_MX_long)
            self.line_XG_long.set_data(np.linspace(40,60,21,dtype='int32'), self.w_XG_long)
            max_height=np.max([self.w_GM_long,self.w_MX_long,self.w_XG_long,self.w_GM_trans,self.w_MX_trans,self.w_XG_trans])
            ...
            if y:
                if x<20:
                    ...
                    idx=round(x)-40
                    if self.button_longitudinal.value=='longitudinal':
                            y_point=self.w_XG_opt_long[idx]
                    else:
                            y_point=self.w_XG_opt_trans[idx]
            ...
            max_height=np.real(np.max(np.r_[self.w_MG_opt_long,self.w_GK_opt_long,self.w_KM_opt_long]))
    ...
    def on_vibration_change(self,*args):
        ...
        if self.button_lattice.value=='square':
            ...
            if self.button_longitudinal.value=='longitudinal':
                if y:
                    if self.button_optic.value=='acoustic':
                        self.point_honey_.set_data((x,self.w_acc_long[self.idx_x_honey][self.idx_y_honey]))
                    else:
                        self.point_honey_.set_data((x,self.w_opt_long[self.idx_x_honey][self.idx_y_honey]))
            else:
                if y:
                    if self.button_optic.value=='acoustic':
                        self.point_honey_.set_data((x,self.w_acc_trans[self.idx_x_honey][self.idx_y_honey]))
                    else:
                        self.point_honey_.set_data((x,self.w_opt_trans[self.idx_x_honey][self.idx_y_honey]))
    ...
    def on_lattice_change(self,*args):
        with self.output_force_constants:
            if self.button_lattice.value=='square':
                ...
                display(VBox([HBox([self.slider_C1_description, self.slider_C1]),HBox([self.slider_C2_description, self.slider_C2])]))
            elif self.button_lattice.value=='honeycomb':
                self.output_force_constants.clear_output()
                display(VBox([HBox([self.slider_C1_description_honey, self.slider_C1_honey]),HBox([self.slider_C2_description_honey, self.slider_C2_honey]),HBox([self.slider_C3_description_honey, self.slider_C3_honey])]))
        ...
        with self.output_branch:
            longitudinal=HBox([self.button_longitudinal_description,self.button_longitudinal])
    ...
    def addArrows(self, *args):
        ...
        def on_frame_change(change):
            ...
            positions2 = positions+self.steps[:,:,:,frame].reshape(-1,3)*self.slider_amp_arrow.value*5
    ...
    def compute_dispersion(self,*args):
        ...
        for i in range(atom_positions_frst_neigh.rows):
            ...
            RHS1+=C1*((exp(I*(K.T.dot(position)))-1)*(u.T@vec)*(vec.T@x))[0]
            RHS2+=C1*((exp(I*(K.T.dot(position)))-1)*(u.T@vec)*(vec.T@y))[0]

        for i in range(atom_positions_scnd_neigh.rows):
            position=atom_positions_scnd_neigh.row(i).T
            vec=position/position.norm()
            RHS1+=C2*((exp(I*(K.T.dot(position)))-1)*(u.T@vec)*(vec.T@x))[0]
            RHS2+=C2*((exp(I*(K.T.dot(position)))-1)*(u.T@vec)*(vec.T@y))[0]
    ...
    def compute_w_A(self,*args):
        ...
        for i in range(61):
            for j in range(61):
                matrice=self.numpy_matrix(self.kx_array[i],self.ky_array[j],self.slider_C1.value,self.slider_C2.value)
    ...
    def compute_trajectory_2D(self, *args):
        ...
        for frame in np.linspace(0, 50, self.nframes):
            ...
            for i,j in product(range(0,10),range(0,10)):
                vec=np.array([[i],[j]])
                step=np.real(self.A*self.slider_amplitude.value*np.exp(1j*(K@vec-self.w*t)))
        ...
        self.replace_trajectory(
            traj=Trajectory(os.path.join(self.tmp_dir.name,"atoms_2d.traj")), representation="spacefill"
        )
    ...
    def onclick(self, event):
        ...
        if np.any(np.all([kx,ky] == np.c_[self.kx_GM,self.ky_GM], axis=1)):
            idx=np.where(np.all([kx,ky] == np.c_[self.kx_GM,self.ky_GM], axis=1))[0][0]
            if self.button_longitudinal.value=='longitudinal':
            ...
            idx=np.where(np.all([kx,ky]== np.c_[self.kx_MX,self.ky_MX], axis=1))[0][0]
            idx+=20
            if self.button_longitudinal.value=='longitudinal':
                self.point_.set_data((idx, self.w_long[self.idx_x][self.idx_y]))
            else :
            ...
            idx=np.where(np.all([kx,ky] == np.c_[self.kx_XG,self.ky_XG], axis=1))[0][0]
            idx+=40
            if self.button_longitudinal.value=='longitudinal':
                self.point_.set_data((idx, self.w_long[self.idx_x][self.idx_y]))
            else :
    ...
    def initialize_paths_bands(self):
        ...
        self.line_GM_trans ,=self.ax_.plot(np.linspace(0,20,21,dtype='int32'),  self.w_GM_trans,c='#1EA896')
        self.line_MX_trans ,=self.ax_.plot(np.linspace(20,40,21,dtype='int32'), self.w_MX_trans,c='#FF0035')
        self.line_XG_trans ,=self.ax_.plot(np.linspace(40,60,21,dtype='int32'), self.w_XG_trans,c='#A11692')

        self.line_GM_long  ,=self.ax_.plot(np.linspace(0,20,21,dtype='int32'),  self.w_GM_long,c='#1EA896')
        self.line_MX_long  ,=self.ax_.plot(np.linspace(20,40,21,dtype='int32'), self.w_MX_long,c='#FF0035')
        self.line_XG_long  ,=self.ax_.plot(np.linspace(40,60,21,dtype='int32'), self.w_XG_long,c='#A11692')
    ...
    def compute_dispersion_honey(self,*args):
        ...
        atom_positions_frst_neigh_1=Matrix([[-1,0],[1/2,-sqrt(3)/2],[1/2,sqrt(3)/2]])
        atom_positions_scnd_neigh_1=Matrix([[3/2,sqrt(3)/2],[-3/2,sqrt(3)/2],[-3/2,-sqrt(3)/2],[3/2,-sqrt(3)/2],[0,sqrt(3)],[0,-sqrt(3)]])
        atom_positions_thrd_neigh_1=Matrix([[-1,sqrt(3)],[-1,-sqrt(3)],[2,0]])

        atom_positions_frst_neigh_2=Matrix([[1,0],[-1/2,-sqrt(3)/2],[-1/2,sqrt(3)/2]])
        atom_positions_scnd_neigh_2=Matrix([[3/2,sqrt(3)/2],[-3/2,sqrt(3)/2],[-3/2,-sqrt(3)/2],[3/2,-sqrt(3)/2],[0,sqrt(3)],[0,-sqrt(3)]])
        ...
        for i in range(atom_positions_frst_neigh_1.rows):
            ...
            vec=position/position.norm()
            RHS1x+=C1*((exp(I*(k.T.dot(position)))*v-u).T@vec*(vec.T@x))[0]          
            RHS1y+=C1*((exp(I*(k.T.dot(position)))*v-u).T@vec*(vec.T@y))[0]

        for i in range(atom_positions_scnd_neigh_1.rows):
            position=atom_positions_scnd_neigh_1.row(i).T
            vec=position/position.norm()
            RHS1x+=C2*((exp(I*(k.T.dot(position)))*u-u).T@vec*(vec.T@x))[0]          
            RHS1y+=C2*((exp(I*(k.T.dot(position)))*u-u).T@vec*(vec.T@y))[0]

        for i in range(atom_positions_thrd_neigh_1.rows):
            position=atom_positions_thrd_neigh_1.row(i).T
            vec=position/position.norm()
            RHS1x+=C3*((exp(I*(k.T.dot(position)))*v-u).T@vec*(vec.T@x))[0]          
            RHS1y+=C3*((exp(I*(k.T.dot(position)))*v-u).T@vec*(vec.T@y))[0]

        for i in range(atom_positions_frst_neigh_2.rows):
            position=atom_positions_frst_neigh_2.row(i).T
            vec=position/position.norm()
            RHS2x+=C1*((exp(I*(k.T.dot(position)))*u-v).T@vec*(vec.T@x))[0]          
            RHS2y+=C1*((exp(I*(k.T.dot(position)))*u-v).T@vec*(vec.T@y))[0]

        for i in range(atom_positions_scnd_neigh_2.rows):
            position=atom_positions_scnd_neigh_2.row(i).T   
            vec=position/position.norm()
            RHS2x+=C2*((exp(I*(k.T.dot(position)))*v-v).T@vec*(vec.T@x))[0]          
            RHS2y+=C2*((exp(I*(k.T.dot(position)))*v-v).T@vec*(vec.T@y))[0]

        for i in range(atom_positions_thrd_neigh_2.rows):
            position=atom_positions_thrd_neigh_2.row(i).T
            vec=position/position.norm()
            RHS2x+=C3*((exp(I*(k.T.dot(position)))*u-v).T@vec*(vec.T@x))[0]          
            RHS2y+=C3*((exp(I*(k.T.dot(position)))*u-v).T@vec*(vec.T@y))[0]
    ...
    def compute_w_A_honey(self,*args):
        ...
        for i in range(61):
            for j in range(61):
                matrice=self.numpy_matrix_honey(self.kx_array_honey[i],self.ky_array_honey[j],self.slider_C1_honey.value,self.slider_C2_honey.value,self.slider_C3_honey.value)
                # matrice[1,1],matrice[2,2]=matrice[2,2],matrice[1,1]
                w1,w2,w3,w4=np.sqrt(np.linalg.eig(matrice)[0])
                v1,v2,v3,v4=np.linalg.eig(matrice.astype('float64'))[1].T
                idx_order=np.argsort((w1,w2,w3,w4)) #Sorts from smallest to largest
    ...
    def compute_trajectory_2D_honey(self, *args):
        ...
        if self.button_optic.value=='optical':
            ...
            if self.button_longitudinal.value=='longitudinal':
                ...
                ux,uy,vx,vy=self.A_acc_trans[self.idx_x_honey][self.idx_y_honey] #ux,vx,uy,vy
        ...
        traj = Trajectory(os.path.join(self.tmp_dir.name,"atoms_2d_honeycomb.traj"), "w")
        ...
        for frame in np.linspace(0, 50, self.nframes):
            ...
            for i,j in product(range(-2,3),range(-2,3)):
                vec=i*ax+j*ay
                step_1=np.real(u*5*self.slider_amplitude.value*np.exp(1j*(k@vec.T-self.w_honey*t)))
            ...
            for i,j in product(range(-2,3),range(-2,3)):
                vec=i*ax+j*ay
                step_2=np.real(v*5*self.slider_amplitude.value*np.exp(1j*(k@vec.T-self.w_honey*t)))
        ...
        self.replace_trajectory(
            traj=Trajectory(os.path.join(self.tmp_dir.name,"atoms_2d_honeycomb.traj")), representation="spacefill"
        )
        # self.view.control.zoom(0.25)
    ...
    def onclick_honey(self, event):
        ...
        # Check if point is on plotted path
        if np.any(np.all(np.isclose([kx,ky],np.c_[self.kx_GK_honey,self.ky_GK_honey]), axis=1)):
            idx=np.where(np.all(np.isclose([kx,ky],np.c_[self.kx_GK_honey,self.ky_GK_honey]), axis=1))[0][0]
            if self.button_optic.value=='acoustic':
                if self.button_longitudinal.value=='longitudinal':
        ...
        elif np.any(np.all(np.isclose([kx,ky],np.c_[self.kx_KM_honey,self.ky_KM_honey]), axis=1)):
            idx=np.where(np.all(np.isclose([kx,ky],np.c_[self.kx_KM_honey,self.ky_KM_honey]), axis=1))[0][0]
            # idx+=10
            if self.button_optic.value=='acoustic':
                if self.button_longitudinal.value=='longitudinal':
        ...
        elif np.any(np.all(np.isclose([kx,ky],np.c_[self.kx_MG_honey,self.ky_MG_honey]), axis=1)):
            idx=np.where(np.all(np.isclose([kx,ky],np.c_[self.kx_MG_honey,self.ky_MG_honey]), axis=1))[0][0]
            # idx+=20
            if self.button_optic.value=='acoustic':
                if self.button_longitudinal.value=='longitudinal':
    ...
    def initialize_2D_band_plot_honey(self):
        ...
        self.ax_honey.plot([0,self.point_honeys_hexagon[3,0]],[0,self.point_honeys_hexagon[3,1]],'--',c='#1EA896',linewidth=2.5)
        self.ax_honey.plot([self.point_honeys_hexagon[3,0],self.point_honeys_hexagon[3,0]],[self.point_honeys_hexagon[3,1],0],'--',c='#FF0035',linewidth=2.5)
        self.ax_honey.plot([self.point_honeys_hexagon[3,0],0],[0,0],'--',c='#A11692',linewidth=2.5)
        ...
        self.ax_honey.set_xticklabels([r'$-\frac{2\pi}{3a}$','','0','',r'$\frac{2\pi}{3a}$'])
        self.ax_honey.set_yticks(np.linspace(-4*np.pi/3/np.sqrt(3),4*np.pi/3/np.sqrt(3),5))
        self.ax_honey.set_yticklabels([r'$-\frac{4\pi}{3\sqrt{3}a}$',r'$-\frac{2\pi}{3\sqrt{3}a}$','0',r'$\frac{2\pi}{3\sqrt{3}a}$',r'$\frac{4\pi}{3\sqrt{3}a}$'])
    ...
    def onclick_honey_(self,event):
        ...
        if self.x_honey_<10:
            ...
            idx_band=np.argmin(np.abs(self.y_honey_-np.c_[self.w_GK_acc_long[idx],self.w_GK_acc_trans[idx],self.w_GK_opt_long[idx],self.w_GK_opt_trans[idx]]))
            if idx_band==0:
            ...
            idx_band=np.argmin(np.abs(self.y_honey_-np.c_[self.w_KM_acc_long[idx],self.w_KM_acc_trans[idx],self.w_KM_opt_long[idx],self.w_KM_opt_trans[idx]]))
            if idx_band==0:
                self.button_optic.value='acoustic'
                self.button_longitudinal.value='longitudinal'
            ...
            idx_band=np.argmin(np.abs(self.y_honey_-np.c_[self.w_MG_acc_long[idx],self.w_MG_acc_trans[idx],self.w_MG_opt_long[idx],self.w_MG_opt_trans[idx]]))
            if idx_band==0:
                self.button_optic.value='acoustic'
                self.button_longitudinal.value='longitudinal'
        ...
        # self.idx_x_honey = (np.abs(self.kx_array_honey - kx[idx])).argmin()
        # self.idx_y_honey = (np.abs(self.ky_array_honey - ky[idx])).argmin()

        self.point_honey_.set_data((round(self.x_honey_),y))
        self.point_honey.set_data((self.kx_array_honey[self.idx_x_honey], self.ky_array_honey[self.idx_y_honey]))
    ...
    def initialize_paths_bands_honey(self):
        ...
        self.line_GK_acc_long ,=self.ax_honey_.plot(self.x_GM,self.w_GK_acc_long,'#1EA896') # GK
        self.line_KM_acc_long ,=self.ax_honey_.plot(self.x_MK,self.w_KM_acc_long,'#FF0035') # KM
        self.line_MG_acc_long ,=self.ax_honey_.plot(self.x_XG,self.w_MG_acc_long,'#A11692') # MG
        self.line_GK_opt_long ,=self.ax_honey_.plot(self.x_GM,self.w_GK_opt_long,'#1EA896') # GK
        self.line_KM_opt_long ,=self.ax_honey_.plot(self.x_MK,self.w_KM_opt_long,'#FF0035') # KM
        self.line_MG_opt_long ,=self.ax_honey_.plot(self.x_XG,self.w_MG_opt_long,'#A11692') # MG

        self.line_GK_acc_trans ,=self.ax_honey_.plot(self.x_GM,self.w_GK_acc_trans,'#1EA896') # GK
        self.line_KM_acc_trans ,=self.ax_honey_.plot(self.x_MK,self.w_KM_acc_trans,'#FF0035') # KM
        self.line_MG_acc_trans ,=self.ax_honey_.plot(self.x_XG,self.w_MG_acc_trans,'#A11692') # MG
        self.line_GK_opt_trans ,=self.ax_honey_.plot(self.x_GM,self.w_GK_opt_trans,'#1EA896') # GK
        self.line_KM_opt_trans ,=self.ax_honey_.plot(self.x_MK,self.w_KM_opt_trans,'#FF0035') # KM
        self.line_MG_opt_trans ,=self.ax_honey_.plot(self.x_XG,self.w_MG_opt_trans,'#A11692') # MG
